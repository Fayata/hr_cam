...
namespace hr_cam
{
    public partial class Report_traffic_detection : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            //    if (Session["site_traffic_detection"] == null)
            //    {
            //        string camera_site = "none";
            //        Session["site_traffic_detection"] = camera_site;
            ...
            //    }
            //    if (Session["from_traffic_detection"] != null)
            //    {
            //    }
            //    else
            //    {
            //        string today = DateTime.Now.ToString("yyyy-MM-dd");
            //        Session["from_traffic_detection"] = today;

            //    }
            //    if (Session["to_traffic_detection"] != null)
            //    {
            //        TextBox2.Text = Session["to_traffic_detection"].ToString();
            //    }
            //    TextBox1.Text = Session["from_traffic_detection"].ToString();
            //    DropDownList1.SelectedValue = Session["site_traffic_detection"].ToString();
            //    FillEventHistory();
            //}
            if (!IsPostBack)
            {
                ...
                //Response.Write("<script>alert('" + Session["site_traffic_detection"].ToString() + "')</script>");
                if (Session["camera_traffic_detection"] == null)
                {
                    string camera = "0";
                    Session["camera_traffic_detection"] = camera;
                    //Response.Write("<script>alert('" + Session["from"].ToString() + "')</script>");

                }
                int jumlahCamera = Convert.ToInt32(Session["camera_traffic_detection"].ToString());
                if (jumlahCamera > 0)
                {
                    if (Session["camera_traffic_detection0"].ToString() == "all")
                    {
                ...
                using (MySqlConnection con = new MySqlConnection(strcon))
                {
                    ...
                    using (MySqlCommand cmd = new MySqlCommand("SELECT c.id, c.name, c.location, cs.name as site from cameras c join camera_sites cs on c.camera_site_id=cs.id where c.name like 'OD%'", con))
                    {
                        using (MySqlDataReader dr = cmd.ExecuteReader())
                        {
                            if (dr.HasRows)
                            {
                                ...
                                {
                                    var sessionValue = Session[$"camera_traffic_detection{i}"];
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            if (selectMultiple != null)
            {
                ...
                foreach (ListItem item in selectMultiple.Items)
                {
                    if (item.Selected) // Memeriksa apakah item dipilih
                    {
                        ...
                        Session["camera_traffic_detection" + i] = item.Value;
                        i++;
                ...
                Session["camera_traffic_detection"] = i;
                //Response.Write("jumlah i:" + Session["camera_traffic_detection"]);
                for (int x = 0; x < i; x++)
                {
                    //Response.Write("camera nya:" + Session["camera_traffic_detection" + x].ToString());
            ...
            Session["from_traffic_detection"] = dari;
            Session["to_traffic_detection"] = ke;
            Response.Redirect("report_traffic_detection.aspx");
        }
        ...
        protected void Button2_Click(object sender, EventArgs e)
        {
            ...
            Session["from_traffic_detection"] = dari;
            Session["to_traffic_detection"] = ke;
            if (Session["camera_traffic_detection"].ToString() == "none")
            {
            ...
            {
                //Response.Redirect("report_traffic_detection_pdf.aspx");
                // Mendapatkan markup HTML dari halaman ExportToPdf.aspx
                StringWriter sw = new StringWriter();
                Server.Execute("report_traffic_detection_pdf.aspx", sw);
        ...
        {
            string camera = Session["camera_traffic_detection"].ToString();
            if (camera != "0")
            {
                int jumlahCamera = Convert.ToInt32(camera);
                string from = Session["from_traffic_detection"].ToString();
                DateTime parsedDateTime = DateTime.Parse(from);
                string from_date = parsedDateTime.ToString("yyyy-MM-dd HH:mm:ss");
                string to = null;
                string to_date = null;
                //Response.Write("<script>alert('Isinya:" + Session["to_traffic_detection"] +"')</script>");
                if (Session["to_traffic_detection"].ToString() != "")
                {
                    //Response.Write("<script>alert('Ga kosong')</script>");
                    to = Session["to_traffic_detection"].ToString();
                ...
                //if (!string.IsNullOrEmpty(Session["status_traffic_detection"].ToString()))
                //{
                //    string status = Session["status_traffic_detection"].ToString();
                //}
                //else
                //{
                //    string status = "all";
                        ...
                        using (MySqlCommand cmd = new MySqlCommand(query, con))
                        {
                            if (jumlahCamera > 0)
                            {
                                ...
                                else
                                {
                                    for (int i = 0; i < jumlahCamera; i++)
                                    {
                                        cmd.Parameters.AddWithValue($"@camera{i}", Session["camera_traffic_detection" + i].ToString());
                                    }
                                }
                            }
                            if (from_date != null)
        ...
        //private void FillEventHistory()
        //{
        //    string camera_site = Session["site_traffic_detection"].ToString();
        //    string from = Session["from_traffic_detection"].ToString();
        //    string to = null;
        //    if (Session["to_traffic_detection"] != null)
        //    {
        ...
        protected void btnExportExcel2_Click(object sender, EventArgs e)
        {
            ...
            Session["from_traffic_detection"] = dari;
            Session["to_traffic_detection"] = ke;
            //if (Session["camera_traffic_detection"] == "none")
            //{
            //    Response.Write("<script>alert('Camera Site cannot be empty')</script>");
            //    //Response.Redirect("report_traffic_detection.aspx");
            //}
        ...
        {
            string fileName = "Report Traffic Detection.csv";
            ...
            if (camera != "0")
            {
                ...
                else
                {
                    using (MySqlConnection con = new MySqlConnection(strcon))
                    {
                        ...
                        query += @" AND ce.type like '%Intrusion Detection%' ORDER BY ce.occurred_at DESC LIMIT 1";
                        //Response.Write(query);
                        using (MySqlCommand cmd = new MySqlCommand(query, con))
                        {
                            if (jumlahCamera > 0)
                            {
                                ...
                                else
                                {
                                    for (int y = 0; y < jumlahCamera; y++)
                                    {
                                        cmd.Parameters.AddWithValue($"@camera{y}", Session["camera_traffic_detection" + y].ToString());
                                    }
                                }
                            }
                            if (from_date != null)
                            ...
                            using (MySqlDataReader dr = cmd.ExecuteReader())
                            {
                                if (dr.HasRows)
                                {
                                    while (dr.Read())
                                    {
                                        //Response.Write("apa"+dr["to_date"].ToString());
                ...
                string csvContent = $"\"Traffic Detection\"\n";
                csvContent += $"\"Periode {per1} to {per2}\"\n";
                csvContent += "\"Image\",\"Occurred At\",\"Location\",\"Site\"\n";
                int x = 0;
                using (MySqlConnection con = new MySqlConnection(strcon))
                {
                    if (con.State == ConnectionState.Closed)
                    ...
                    else
                    {
                        ...
                        query += @" AND ce.type like '%Intrusion Detection%' ORDER BY ce.occurred_at ASC";
                        using (MySqlCommand cmd = new MySqlCommand(query, con))
                        {
                            if (jumlahCamera > 0)
                            {
                            ...
                            using (MySqlDataReader dr = cmd.ExecuteReader())
                            {
                                if (dr.HasRows)
                                {
                                    while (dr.Read())
                                    {
                                        ...
                                        csvContent += $"\"{link1}\",\"{occurred_at}\",\"{location}\",\"{site}\"\n";
                                    }
                                    dr.Close();
                                }
                                else
        ...
        {
            string fileName = "Report Traffic Detection.xls";
            ...
            if (camera != "0")
            {
                ...
                <tr>
                    <td colspan='5' align='center'><b>Report Traffic Detection</b></td>
                </tr>
                <tr>
                    <td colspan='5' align='center'><b>Periode {per1} to {per2}</b></td>
                </tr>
                <tr>
                    <td><b>No</b></td>
                    <td><b>Image</b></td>
            ...
            if (camera != "0")
            {
                ...
                using (MySqlConnection con = new MySqlConnection(strcon))
                {
                    ...
                    else
                    {
                        //Response.Write(fromDate);
                        //Response.Write("SELECT ce.camera_id, ce.type AS event_type,ce.plate_number_file, ce.occurred_at, ce.person_identification_number, ce.image_file, c.location, p.name as person, p.gender, p.type as person_type, cs.name AS site, ce.image_file, ce.plate_number_file FROM camera_events as ce JOIN cameras as c ON ce.camera_id = c.name LEFT JOIN persons as p ON ce.person_identification_number = p.identification_number JOIN camera_sites AS cs ON c.camera_site_id=cs.id  WHERE cs.id=@camera_site AND (ce.image_file != '' OR ce.image_file != NULL) AND DATE(ce.occurred_at) >="+from+" AND DATE(ce.occurred_at) <="+to+" AND ce.type like '%Plate%' ORDER BY ce.occurred_at ASC");
                        string query = @"SELECT ce.camera_id, ce.type AS event_type,ce.plate_number_file, ce.occurred_at, ce.person_identification_number, ce.image_file, c.location, p.name as person, p.gender, p.type as person_type, cs.name AS site, ce.image_file, ce.plate_number_file, vh.entry_code,v.plate_number FROM camera_events as ce JOIN cameras as c ON ce.camera_id = c.name LEFT JOIN persons as p ON ce.person_identification_number = p.identification_number JOIN camera_sites AS cs ON c.camera_site_id=cs.id LEFT JOIN traffics v on ce.plate_number=v.plate_number LEFT JOIN traffic_history vh ON v.id=vh.traffic_id AND vh.created_at = (SELECT MAX(vh2.created_at) FROM traffic_history vh2 WHERE vh2.traffic_id = v.id AND vh2.created_at <= ce.occurred_at) WHERE (ce.image_file != '' OR ce.image_file != NULL) ";
                        if (jumlahCamera > 0)
                        {
                            if (Session["camera_traffic_detection0"].ToString() == "all")
                            {
