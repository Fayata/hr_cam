...
namespace hr_cam
{
    public partial class dpo : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            //{
            //    // Memeriksa apakah permintaan berisi file
            //    if (Request.Files.Count > 0)
            //    {
            //        HttpPostedFile file = Request.Files[0]; // Mengambil file dari permintaan
            //                                                // Lakukan apa pun yang perlu dilakukan dengan file, misalnya, menyimpannya ke server
            //        file.SaveAs(Server.MapPath("~/FileCSV/" + file.FileName));
            //        // Tanggapi ke klien bahwa file telah diterima dan disimpan
            //        Response.Write("File berhasil diterima dan disimpan.");
            //    }
            //    else
            //    {
            //        // Jika tidak ada file yang dikirimkan, tanggapi dengan pesan kesalahan
            //        Response.StatusCode = 400; // Bad Request
            //        Response.Write("Tidak ada file yang diterima.");
            //    }
            //    Response.End();
            //}
            if (!IsPostBack)
        ...
        private void FillTableFromDatabase()
        {
            using (MySqlConnection con = new MySqlConnection(strcon))
            {
                ...
                using (MySqlCommand cmd = new MySqlCommand("SELECT * from persons where type like '%blacklist%'", con))
                {
                    using (MySqlDataReader dr = cmd.ExecuteReader())
                    {
                        while (dr.Read())
                        {
                            ...
                            string deleteUrl = "delete_dpo.aspx?id=" + personId;
        ...
        public static async Task<string> UploadFile()
        {
            try
            {
                ...
                if (file != null && file.ContentLength > 0)
                {
                    ...
                    {
                        client.BaseAddress = new Uri("https://localhost:3000/dpo/import");
