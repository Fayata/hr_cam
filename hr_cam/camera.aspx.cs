    ...
    public partial class camera : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    //Response.Write("<script>alert('You can not access Cameras, it can only be accessed by admin')</script>");
        ...
        private void FillTableFromDatabase()
        {
            using (MySqlConnection con = new MySqlConnection(strcon))
            {
                ...
                using (MySqlCommand cmd = new MySqlCommand("SELECT c.id, c.name, c.location, c.url, c.username, cs.name as site FROM cameras c join camera_sites cs on c.camera_site_id=cs.id", con))
                {
                    using (MySqlDataReader dr = cmd.ExecuteReader())
                    {
                        while (dr.Read())
                        {
                            ...
                            string cameraId = dr["id"].ToString();
                            ...
                            TableCell locationCell = new TableCell() { Text = location };
                            ...
                            string editUrl = "edit_camera.aspx?id=" + cameraId;
                            string deleteUrl = "delete_camera.aspx?id=" + cameraId;
