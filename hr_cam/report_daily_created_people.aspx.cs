...
namespace hr_cam
{
    public partial class report_daily_created_people : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                //if (Session["from_daily_created_people"] != null)
                //{
                //}
                //else
                //{
                //    string today = DateTime.Now.ToString("yyyy-MM-dd 00:00");
                //    Session["from_daily_created_people"] = today;

                //}
                //if (Session["to_daily_created_people"] != null)
                //{
                //    TextBox2.Text = Session["to_daily_created_people"].ToString();
                //}
                //TextBox1.Text = Session["from_daily_created_people"].ToString();
                FillEventHistory();
            }
        }
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            //Session["from_daily_created_people"] = dari;
            //Session["to_daily_created_people"] = ke;
            Response.Redirect("report_daily_created_people.aspx");
        }
        ...
        protected void Button2_Click(object sender, EventArgs e)
        {
            ...
            //Response.Redirect("report_daily_created_people_pdf.aspx");
            // Mendapatkan markup HTML dari halaman ExportToPdf.aspx
            StringWriter sw = new StringWriter();
            Server.Execute("report_daily_created_people_pdf.aspx", sw);
            ...
            Response.AppendHeader("Content-Disposition", "attachment; filename=Report Daily Created People.pdf");
        ...
        protected void btnExportExcel_Click(object sender, EventArgs e)
        {
            ...
            //Session["from_daily_created_people"] = dari;
            //Session["to_daily_created_people"] = ke;
            //if (Session["camera_daily_created_people"].ToString() == "none")
            //{
        ...
        {
            string fileName = "Report Daily Created People.csv";
            ...
            string csvContent = $"\"Report Daily Created People\"\n";
            csvContent += "\"Image\",\"Identification Number\",\"Name\",\"Gender\",\"Type\",\"Created At\"\n";
                    ...
                    using (MySqlDataReader dr = cmd.ExecuteReader())
                    {
                        if (dr.HasRows)
                        {
                            while (dr.Read())
                            {
                                ...
                                csvContent += $"\"{link1}\",\"{identification_number}\",\"{name}\",\"{gender}\",\"{type}\",\"{created_at}\"\n";
        ...
        {
            string fileName = "Report People.xls";
            string camera = Session["camera_daily_created_people"].ToString();
            if (camera != "0")
            {
                ...
                else
                {
                    using (MySqlConnection con = new MySqlConnection(strcon))
                    {
                        ...
                        if (jumlahCamera > 0)
                        {
                            if (Session["camera_daily_created_people0"].ToString() == "all")
                            {
                        ...
                        using (MySqlCommand cmd = new MySqlCommand(query, con))
                        {
                            if (jumlahCamera > 0)
                            {
                                ...
                                else
                                {
                                    for (int y = 0; y < jumlahCamera; y++)
                                    {
                                        cmd.Parameters.AddWithValue($"@camera{y}", Session["camera_daily_created_people" + y].ToString());
                                    }
                                }
                            }
                ...
                string tableHtml = $@"<table border='1'>
                        <tr>
                            <td colspan='7' align='center'><b>Report People</b></td>
                        </tr>
                        <tr>
        ...
        protected void ExportToExcelWithImages2()
        {
            ...
            ISheet sheet = workbook.CreateSheet("Report People");
            ...
            if (camera != "0")
            {
                ...
                using (MySqlConnection con = new MySqlConnection(strcon))
                {
                    ...
                    else
                    {
                        ...
                        headerCell1.SetCellValue("Report People");
                        ...
                        using (MemoryStream stream = new MemoryStream())
                        {
                            ...
                            Response.Clear();
                            Response.Buffer = true;
                            Response.AddHeader("content-disposition", "attachment;filename=ReportPeople.xlsx");
