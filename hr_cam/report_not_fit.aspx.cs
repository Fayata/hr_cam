...
namespace hr_cam
{
    public partial class report_not_fit : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            //    if (Session["site_not_fit"] == null)
            //    {
            //        string camera_site = "none";
            //        Session["site_not_fit"] = camera_site;
            ...
            //    }
            //    if (Session["from_not_fit"] != null)
            //    {
            //    }
            //    else
            //    {
            //        string today = DateTime.Now.ToString("yyyy-MM-dd");
            //        Session["from_not_fit"] = today;

            //    }
            //    if (Session["to_not_fit"] != null)
            //    {
            //        TextBox2.Text = Session["to_not_fit"].ToString();
            //    }
            //    TextBox1.Text = Session["from_not_fit"].ToString();
            //    DropDownList1.SelectedValue = Session["site_not_fit"].ToString();
            //    FillEventHistory();
            //}
            ...
            if (!IsPostBack)
            {
                ...
                //Response.Write("<script>alert('" + Session["site_not_fit"].ToString() + "')</script>");
                if (Session["camera_not_fit"] == null)
                {
                    string camera = "0";
                    Session["camera_not_fit"] = camera;
                    //Response.Write("<script>alert('" + Session["from"].ToString() + "')</script>");

                }
                int jumlahCamera = Convert.ToInt32(Session["camera_not_fit"].ToString());
                if (jumlahCamera > 0)
                {
                    if (Session["camera_not_fit0"].ToString() == "all")
                    {
                    ...
                    using (MySqlCommand cmd = new MySqlCommand("SELECT c.id, c.name, c.location, cs.name as site from cameras c join camera_sites cs on c.camera_site_id=cs.id where c.name not like 'LPR%'", con))
                    {
                        using (MySqlDataReader dr = cmd.ExecuteReader())
                        {
                            if (dr.HasRows)
                            {
                                ...
                                {
                                    var sessionValue = Session[$"camera_not_fit{i}"];
                ...
                TextBox1.Text = Session["from_not_fit"].ToString();
                if (Session["status_not_fit"] != null)
                {
                    DropdownStatus.SelectedValue = Session["status_not_fit"].ToString();
                }
                else
                {
                    string status = "All";
                    Session["status_not_fit"] = status;
                }
                FillEventHistory();
            }
        }
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            if (selectMultiple != null)
            {
                ...
                foreach (ListItem item in selectMultiple.Items)
                {
                    if (item.Selected) // Memeriksa apakah item dipilih
                    {
                        ...
                        Session["camera_not_fit" + i] = item.Value;
                        i++;
                ...
                Session["camera_not_fit"] = i;
                //Response.Write("jumlah i:" + Session["camera_not_fit"]);
                for (int x = 0; x < i; x++)
                {
                    Response.Write("camera nya:" + Session["camera_not_fit" + x].ToString());
            ...
            Session["status_not_fit"] = status;
            Session["from_not_fit"] = dari;
            Session["to_not_fit"] = ke;
            Response.Redirect("report_not_fit.aspx");
        }
        ...
        protected void Button2_Click(object sender, EventArgs e)
        {
            ...
            Session["from_not_fit"] = dari;
            Session["to_not_fit"] = ke;
            if (Session["camera_not_fit"].ToString() == "0")
            {
            ...
            {
                //Response.Redirect("report_not_fit_pdf.aspx");
                // Mendapatkan markup HTML dari halaman ExportToPdf.aspx
                StringWriter sw = new StringWriter();
                Server.Execute("report_not_fit_pdf.aspx", sw);
                ...
                Response.AppendHeader("Content-Disposition", "attachment; filename=Report Face Recognition Invalid or Exception.pdf");
        ...
        {
            string camera = Session["camera_not_fit"].ToString();
            if (camera != "0")
            {
                int jumlahCamera = Convert.ToInt32(camera);
                string from = Session["from_not_fit"].ToString();
                DateTime parsedDateTime = DateTime.Parse(from);
                string from_date = parsedDateTime.ToString("yyyy-MM-dd HH:mm:ss");
                string to = null;
                string to_date = null;
                string filter = Session["status_not_fit"].ToString();
                //Response.Write("<script>alert('Isinya:" + Session["to_not_fit"] +"')</script>");
                if (Session["to_not_fit"].ToString() != "")
                {
                    //Response.Write("<script>alert('Ga kosong')</script>");
                    to = Session["to_not_fit"].ToString();
                ...
                using (MySqlConnection con = new MySqlConnection(strcon))
                {
                    ...
                    else
                    {
                        ...
                        if (filter != null)
                        {
                            if (filter != "All")
                            {
                                query += @" AND ph.entry_code=@filter";
                            }
                        }
                        if (from_date != null)
                        {
                        ...
                        using (MySqlCommand cmd = new MySqlCommand(query, con))
                        {
                            if (jumlahCamera > 0)
                            {
                                ...
                                else
                                {
                                    for (int i = 0; i < jumlahCamera; i++)
                                    {
                                        cmd.Parameters.AddWithValue($"@camera{i}", Session["camera_not_fit" + i].ToString());
                                    }
                                }
                            }
                            if (filter != null)
        ...
        //private void FillEventHistory()
        //{
        //    string camera_site = Session["site_not_fit"].ToString();
        //    string from = Session["from_not_fit"].ToString();
        //    string to = null;
        //    if (Session["to_not_fit"] != null)
        //    {
        ...
        {
            string fileName = "Report_Face_Recognition_Invalid_or_Exception.csv";
            string camera = Session["camera_not_fit"].ToString();
            if (camera != "0")
            {
                int jumlahCamera = Convert.ToInt32(camera);
                        ...
                        using (MySqlCommand cmd = new MySqlCommand(query, con))
                        {
                            if (jumlahCamera > 0)
                            {
                                ...
                                else
                                {
                                    for (int y = 0; y < jumlahCamera; y++)
                                    {
                                        cmd.Parameters.AddWithValue($"@camera{y}", Session["camera_not_fit" + y].ToString());
                                    }
                                }
                            }
                            if (filter != null)
                ...
                string csvContent = $"\"Report Face Recognition Invalid or Exception\"\n";
        ...
        {
            string fileName = "Report Face Recognition Invalid or Exception.xls";
            string camera = Session["camera_not_fit"].ToString();
            if (camera != "0")
            {
                int jumlahCamera = Convert.ToInt32(camera);
                ...
                <tr>
                    <td colspan='8' align='center'><b>Report Face Recognition Invalid or Exception</b></td>
                </tr>
                <tr>
        ...
        protected void ExportToExcelWithImages2()
        {
            ...
            ISheet sheet = workbook.CreateSheet("Report FR Invalid or Exception");
                ...
                using (MySqlConnection con = new MySqlConnection(strcon))
                {
                    ...
                    else
                    {
                        ...
                        using (MemoryStream stream = new MemoryStream())
                        {
                            ...
                            Response.Clear();
                            Response.Buffer = true;
                            Response.AddHeader("content-disposition", "attachment;filename=ReportFaceRecognitionInvalidorException.xlsx");
