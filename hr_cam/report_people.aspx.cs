    ...
    public partial class Report_people : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                //Response.Write("<script>alert('" + Session["site_people"].ToString() + "')</script>");
                if (Session["camera_people"] == null)
                {
                    string camera = "0";
                    Session["camera_people"] = camera;
                    //Response.Write("<script>alert('" + Session["from"].ToString() + "')</script>");

                }
                int jumlahCamera = Convert.ToInt32(Session["camera_people"].ToString());
                if (jumlahCamera > 0)
                {
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            if (selectMultiple != null)
            {
                ...
                Session["camera_people"] = i;
                for (int x = 0; x < i; x++)
                {
                    //Response.Write("camera nya:" + Session["camera_people" + x].ToString());
        ...
        protected void Button2_Click(object sender, EventArgs e)
        {
            ...
            if (selectMultiple != null)
            {
                ...
                Session["camera_people"] = i;
                //Response.Write("jumlah i:" + Session["camera_people"]);
                for (int x = 0; x < i; x++)
                {
                    //Response.Write("camera nya:" + Session["camera_people" + x].ToString());
            ...
            Session["from_people"] = dari;
            Session["to_people"] = ke;
            if (Session["camera_people"].ToString() == "none")
            {
            ...
            else
            {
                ...
                Response.AppendHeader("Content-Disposition", "attachment; filename=Report Unique Face Recognition.pdf");
        ...
        {
            string fileName = "Report Unique Face Recognition.csv";
            string camera = Session["camera_people"].ToString();
            if (camera != "0")
            {
                int jumlahCamera = Convert.ToInt32(camera);
                        ...
                        using (MySqlCommand cmd = new MySqlCommand(query, con))
                        {
                            if (jumlahCamera > 0)
                            {
                                ...
                                else
                                {
                                    for (int y = 0; y < jumlahCamera; y++)
                                    {
                                        cmd.Parameters.AddWithValue($"@camera{y}", Session["camera_people" + y].ToString());
                                    }
                                }
                            }
                ...
                string csvContent = $"\"Report Unique Face Recognition\"\n";
                csvContent += $"\"Periode {per1} to {per2}\"\n";
                csvContent += "\"Image\",,\"First Occurred At\",\"Location\",\"Site\",\"Person\"\n";
                            ...
                            using (MySqlDataReader dr = cmd.ExecuteReader())
                            {
                                if (dr.HasRows)
                                {
                                    while (dr.Read())
                                    {
                                        ...
                                        csvContent += $"\"{link1}\",\"{link2}\",\"{occurred_at}\",\"{location}\",\"{site}\",\"{person}\"\n";
        ...
        {
            string fileName = "Report Unique Face Recognition.xls";
            string camera = Session["camera_people"].ToString();
            if (camera != "0")
            {
                int jumlahCamera = Convert.ToInt32(camera);
                ...
                string tableHtml = $@"<table border='1'>
                        <tr>
                            <td colspan='7' align='center'><b>Report Unique Face Recognition</b></td>
                        </tr>
                        <tr>
        ...
        protected void ExportToExcelWithImages2()
        {
            ...
            ISheet sheet = workbook.CreateSheet("Report Unique Face Recognition");
            ...
            if (camera != "0")
            {
                ...
                using (MySqlConnection con = new MySqlConnection(strcon))
                {
                    ...
                    else
                    {
                        ...
                        headerCell1.SetCellValue("Report Unique Face Recognition");
